...
import {
  ...
  summary,
  verification
} from "./detectionGapCoverageLabService.js";
...
<html lang="en">
  ...
  <body>
    ...
    <div class="wrap">
      <div class="topbar">
        <div class="left">Kinetic Gain · Detection Gap Coverage Lab</div>
        <div class="right">
          <div>synthetic coverage surfaces · incident packets</div>
          <div>vendor-neutral secops · detection engineering · soc coverage</div>
        </div>
      </div>
      <div class="herorow">
        <section class="hero">
          <div class="chiprow">
            ...
            <span class="meta-chip">Vendor-neutral detection coverage proof</span>
            <span class="meta-chip">Synthetic surface + control exports</span>
          </div>
          <h1>Detection coverage that stays operator-readable.</h1>
          <p>This control plane turns SecOps coverage data into one buyer-readable surface: telemetry health, control coverage, automation readiness, stale incidents, and the response packets needed before SOC drift, audits, or trust posture slip.</p>
          <div class="navrow">
        ...
        <aside class="side">
          <div class="bluf">
            ...
            <p><strong>Enterprise detection coverage for SOC, identity, endpoint, and collaboration teams.</strong><br />Audit-safe visibility into telemetry gaps, rule drift, and incident automation posture without exposing raw admin surfaces or credentials.</p>
          </div>
          <div class="corr">
            <div class="lbl">Proof Layer</div>
            <p><strong>Offline analyzer + CLI + dashboard surface.</strong><br />This repo includes a reusable analyzer that reads detection coverage snapshots and turns them into surface, gap, and incident packets.</p>
          </div>
          <div class="corr">
            <div class="lbl">Why it matters</div>
            <p>Recruiters looking for <strong>SecOps / detection engineering / SIEM / XDR / SOC</strong> should see real operator work: telemetry gaps, control coverage, playbook drift, and incident-response sequencing.</p>
          </div>
        </aside>
      </div>
      ${body}
      <div class="footer">
        <div>detection-gap-coverage-lab · synthetic sample data only</div>
...
export function renderOverview() {
  ...
  return layout(
    ...
    `<section class="section">
        ...
        <div class="kpis">
          <div class="kpi cyan"><div class="v">${metrics.surfaces}</div><div class="lbl">surfaces</div><div class="h">Synthetic detection surfaces across global and regional scopes.</div></div>
          <div class="kpi green"><div class="v">${metrics.healthySurfaces}</div><div class="lbl">healthy surfaces</div><div class="h">Surfaces currently carrying healthy detection coverage.</div></div>
          <div class="kpi plum"><div class="v">${metrics.gaps}</div><div class="lbl">gaps</div><div class="h">Coverage gaps across identity, endpoint, collaboration, and incident posture.</div></div>
          <div class="kpi red"><div class="v">${metrics.highSeverityGaps}</div><div class="lbl">high gaps</div><div class="h">High-severity coverage findings needing the fastest operator path.</div></div>
          ...
          <div class="kpi red"><div class="v">${metrics.staleGaps}</div><div class="lbl">stale active gaps</div><div class="h">Findings that have remained open longer than the incident SLA.</div></div>
        </div>
      </section>
      <section class="section">
        ...
        <div class="stack">
          ...
          <div class="src"><div class="src-name">operator evidence</div><div class="src-tit">Turn coverage exports into control-plane proof</div><p>Every lane stays tied to owner, detection focus, surface health, and the next concrete operator move.</p></div>
          <div class="src"><div class="src-name">recruiter signal</div><div class="src-tit">Show real SecOps depth</div><p>This is real detection-coverage and incident-operations proof, not generic cloud-security copy.</p></div>
        </div>
      </section>`
  );
}
...
export function renderDetectionLane() {
  return layout(
    "Detection Gap Coverage Lab — Detection Lane",
    "/detection-lane",
    `<section class="section">
...
export function renderCoverageGaps() {
  return layout(
    "Detection Gap Coverage Lab — Coverage Gaps",
    "/coverage-gaps",
    `<section class="section">
...
export function renderIncidentPosture() {
  return layout(
    "Detection Gap Coverage Lab — Incident Posture",
    "/incident-posture",
    `<section class="section">
...
export function renderVerification() {
  return layout(
    "Detection Gap Coverage Lab — Verification",
    "/verification",
    `<section class="section">
        ...
        <div class="stack">
          ${verification()
            .map(
              (item, index) => `<div class="src"><div class="src-name">verification ${index + 1}</div><div class="src-tit">${item}</div><p>This surface is built to stay honest about offline exports, synthetic sample data, and real SecOps detection posture.</p></div>`
            )
            .join("")}
        </div>
      </section>`
...
export function renderDocs() {
  return layout(
    "Detection Gap Coverage Lab — Docs",
    "/docs",
    `<section class="section">
        ...
        <div class="stack">
          ...
          <div class="src"><div class="src-name">cli</div><div class="src-tit">Offline detection analysis</div><p><code>npx detection-gap-coverage fixtures/detection-coverage-clean.json --format summary</code> renders the same incident posture the dashboard exposes.</p></div>
        </div>
      </section>`
  );
}
